...
### Running function `myplot()` to create a scatter plot
...
### Running function `myplot()` to create a violin plot with `plot2==FALSE`
...
### Running function `myplot()` to create a box plot with `jitter=="TRUE"` and `plot2=="TRUE"`

```{r}
xlab <- " "
ylab <- "gdpPercap"
title <- "Gapminder: gdpPercap across all continents (Jitter and Plotly turned on)"
myplot(gapminder, "box", gapminder$lifeExp, gapminder$gdpPercap, gapminder$continent, "TRUE", "TRUE", xlab, ylab, title, "FALSE")
```
...
From the above output we see that the trump_tweets_df is a ‘tbl_df’ inherited from class ‘tbl’ and 'data.frame'. We can also see that it has	1512 obs (rows) and  16 variables (columns) prefixed with a `$`. In the created function, I also use the function `sapply()` to help me derive each column name and datatype as this data is helpful when working with `purr` functions like `map()`

### Working with str_detect(), str_split(), str_replace(), substring(), gregexpr

str_detect() is used to check whether a string (word or group of words) exists in a string or character vector like  sentences we commonly use. First, I will subset the data to a small piece that I can work with.
...
str(tweets) #character of 7 elements
```
...
#### A sneak peak into the contents of tweets
```{r}
tweets
```

#### Deriving element at index 2 and the number of distinct words therein - str_split() and str_detect()
```{r}
tweets[2]
...
The above line shows the second element in the tweets character (the words in this element will further be used in following sections) we can also see by the application of `str_split` that the number of words are 23. Let's say, I have these many words or a sentence, and I would like to find a word or words in a string, to a accomplish this, we can apply the `str_detect()` function to find words for example `economic policy` and `Morell` in  tweets[2] as seen in the code below.
...
From the above output, if the pattern being searched for is found in the string, R returns `TRUE`, otherwise it will return `FALSE`. We can also replace words as seen in the code snippet below:
...
We can see that the word `former` as seen in tweets[2] `Michael Morell, the lightweight former Acting Director of C.I.A., and a man who has made serious bad calls, is a total Clinton flunky!"` has been replaced with  `current`. A similar function to str_replace is the substring() function that provides replacement and more operations, however the use of substring() doese replacement based on position as shown below.


#### Working with substring() 
The substring function is a powerful R function that can be used either to extract parts of character string, or to change the values of parts of character strings. For example running `substring("STAT547M is awesome",1,4)` would return `STAT`
...
```
Notice that "funny" is being replaced with "weird" in output line one. This works perfectly because the replacing and replaced elements are of the same length. In output line 2, however, we see that if the replacing element has length > the the element to be replaced, extra chatacters are truncated.
...
At certain points in time, we may need to work with non rectangular data such as that generated by JSON or XML. We can apply Purr's map() functions to dig information out of non-rectangular data structures and create neat data frames. 
In the following sections, I walk you through the following funcitons: `map()`, `unnest()` enframe()
...
The above output shows us that found is of type and class `list` and when run through the `str()` function shows that the variable `found` has 4 elements when searched through tweets. Here each element is an integer vector and shows -1  if no match is found. 
...
From the above output, we see that gh_users is a list with 6 main elements `login`, `id`, `avatar_url`, `gravatar_id`, `url` and `html_url`. We can also see that it is not easy to map various elements together. We can use `map()` to reach into each  list (a user's list) and map an element of interest to another element say username. This is shown in the code snippet below. We can also test other datatypes like with map_lgl() for logical values, map_int() for integers and map_dbl() for doubles In the code snippet below, the 18th element of each user’s list is his or her name and we get them like so: `map(gh_users, 18)`
...
In instances where we need to extract multiple elements we can use square bracket indexing and the character vector of the names of our desired elements
...
Extract multiple elements per user by mapping [
...
### Working with map_df()
Creating a neat data frame by taking one row per user and  `login`, `id`, `avatar_url`, `gravatar_id`, `url` and `html_url` as variables. To do this we use the map_df() function
...
The above table is the neat dataframe created using map_df. Let us look into the data types of each of the variables in the created dataframe.
...
After parsing the user names to variable names, we can now create a dataframe as shown in the code snippet below:
...
From the above output we see that the created dataframe is of class `tbl_df` inherited from classes `tbl` and `data.frame`. The tibble has 6 observations (rows) and 2 variables (colums): username and gh_repos which are of type character and list respectively.
